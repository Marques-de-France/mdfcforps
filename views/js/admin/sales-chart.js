(function () {
  ...
  function normalizeAnalytics(raw) {
    ...
    if (Array.isArray(raw.data)) {
      return raw.data.map(function (item) {
        ...
        var conversions = Number(item.conversions || 0);
        return { date: date, revenue: revenue, conversions: conversions };
      });
    }

    var months = Array.isArray(raw.months) ? raw.months : [];
    var revenues = Array.isArray(raw.revenues) ? raw.revenues : [];
    var salesCounts = Array.isArray(raw.salesCounts) ? raw.salesCounts : [];

    return months.map(function (label, index) {
      return {
        date: String(label || ''),
        revenue: Number(revenues[index] || 0),
        conversions: Number(salesCounts[index] || 0),
      };
    });
  }
  ...
  function aggregateRows(rows, granularity) {
    ...
    rows.forEach(function (row) {
      ...
      if (!groups[key]) {
        groups[key] = { key: key, revenue: 0, conversions: 0, dateObj: toDateFromKey(key) };
      }

      groups[key].revenue += Number(row.revenue || 0);
      groups[key].conversions += Number(row.conversions || 0);
    });
  ...
  function buildDenseSeries(rows, granularity, bounds) {
    if (!bounds || !bounds.start || !bounds.end) {
      return aggregateRows(rows, granularity);
    }
  ...
  function applyFallbackStyles(canvas, tooltip, rangeSelect, granularitySelect) {
    ...
    if (window.matchMedia && window.matchMedia('(max-width: 600px)').matches && container) {
  ...
  function initSalesChart() {
    var config = window.mdfcforpsSales || {};
    var canvas = document.getElementById('mdf-sales-revenue-chart');
    var tooltip = document.getElementById('mdf-sales-chart-tooltip');
    var emptyEl = document.getElementById('mdf-sales-chart-empty');
    var revenueEl = document.getElementById('mdf-sales-kpi-revenue');
    var salesEl = document.getElementById('mdf-sales-kpi-sales');
    var rangeSelect = document.getElementById('mdf-sales-range');
    var granularitySelect = document.getElementById('mdf-sales-granularity');
    ...
    var state = {
      range: rangeSelect ? rangeSelect.value : '28d',
      granularity: granularitySelect ? granularitySelect.value : 'day',
      points: [],
    };
